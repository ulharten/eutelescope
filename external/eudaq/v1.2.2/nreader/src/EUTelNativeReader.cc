...
EUTelNativeReader::EUTelNativeReader()
  ...
  registerOptionalParameter("SkipNEvents",
                            "Number of events to skip at beginning",
                            _skipNEvents, static_cast<int>(1));
...
//    nextNextEudaqEvent = reader->Event();
    ...
    } else if ( eventCounter < _skipNEvents ) {
      ++eventCounter;
      continue;
    }
    else {

      LCEvent *lcEvent = eudaq::PluginManager::ConvertToLCIO(&eudaqEvent, &nextEudaqEvent/*, &nextNextEudaqEvent*/);
