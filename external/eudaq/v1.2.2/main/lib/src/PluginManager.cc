...
namespace eudaq {
  ...
  lcio::LCEvent *PluginManager::ConvertToLCIO(DetectorEvent *dev, DetectorEvent *next) {
    lcio::LCEventImpl *event = new lcio::LCEventImpl;
    event->setEventNumber(dev->GetEventNumber());
    event->setRunNumber(dev->GetRunNumber());
    event->setTimeStamp(dev->GetTimestamp());
    ...
    for (size_t i = 0; i < dev->NumEvents(); ++i) {
      if ( i == 2 ) 
      	ConvertLCIOSubEvent(*event, dev->GetEvent(i), next->GetEvent(i));
      else 
	ConvertLCIOSubEvent(*event, *dev->GetEvent(i));
    }
  ...
  lcio::LCEvent *PluginManager::ConvertToLCIO(DetectorEvent *dev, DetectorEvent *next, DetectorEvent *nextnext) {
    lcio::LCEventImpl *event = new lcio::LCEventImpl;
    event->setEventNumber	(dev->GetEventNumber());
    event->setRunNumber		(dev->GetRunNumber());
    event->setTimeStamp		(dev->GetTimestamp());
//	NumEvents()=2 <=> mupix event (0, 1 for trigger/mimosa)
    for (size_t i = 0; i < dev->NumEvents(); ++i) {
      if ( i == 2 )
      {
      	ConvertLCIOSubEvent(*event, dev->GetEvent(i), next->GetEvent(i), nextnext->GetEvent(i));
      }
      else
      {
    	ConvertLCIOSubEvent(*event, *dev->GetEvent(i));
  ...
  void PluginManager::ConvertLCIOSubEvent(lcio::LCEvent &dest,
                                          const Event *source,
					  const Event *next_source) {
    GetInstance().GetPlugin(*source).GetLCIOSubEvent(dest, source, next_source);
  }
  void PluginManager::ConvertLCIOSubEvent(lcio::LCEvent &dest,
                                          const Event *prev_source,
										  const Event *source,
										  const Event *next_source) {
    GetInstance().GetPlugin(*source).GetLCIOSubEvent(dest, prev_source, source, next_source);
  }
